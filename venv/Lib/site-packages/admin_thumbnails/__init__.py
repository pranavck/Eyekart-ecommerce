...
def thumbnail(field_name, *args, **kwargs):
    ...
    def _model_admin_wrapper(admin_class):
        ...
        def thumbnail_field(self, obj):
            if isinstance(getattr(type(obj), field_name, None), property) or isinstance(
                getattr(type(obj), field_name, None), cached_property
            ):  # noqa: E501
                ...
                if not isinstance(value, FieldFile):
                    raise TypeError(
                        ...
                        supplied field name, but it did not return an instance of
                        `FieldFile` or a descendent. If using a property as the
                        thumbnail source, it must return a value from a field that
                        is an instance of Django's `ImageField`, `FileField` or
                        easy_thumbnails' `ThumbnailerImageField`
                        (received: {0})""".format(
                            type(value)
                        )
                    )
            ...
            elif ThumbnailerImageField and isinstance(field, ThumbnailerImageField):
                raise NameError(
                    """admin_thumbnails: If using a `ThumbnailerImageField` you must
                    specify a `THUMBNAIL_ALIASES` alias named `admin_thumbnail` to
                    use when generating thumbnails, or use the
                    `ADMIN_THUMBNAIL_THUMBNAIL_ALIAS` setting to specify a different
                    alias to use"""
                )
        ...
        new_field_name = "{0}{1}".format(field_name, ADMIN_THUMBNAIL_FIELD_SUFFIX)
